...
def random_call(person, suite, number):
    ...
    if max(spades, hearts, diamonds, flowers) == spades:
        sui = 0
    if int(number) >= max(spades, hearts, diamonds, flowers)-2:
        sui = suite
        num = number
        result = "pass"
        person.call_status = int(1)
...
def play(position, players, king, model, close_show, num_strategy1, num_strategy2, nickname="國家機器"):  # 玩家的名字 待改
    ...
    if model == 0:  # 玩家和三名電腦對戰
        ...
        if person_on_turn.name in ('國家機器', '國家機器的助手'):
            fst_card = person_on_turn.fst_turn_decide(num_strategy1, person_on_turn, king)
        else:
            fst_card = random_choose(person_on_turn,
                                 len(person_on_turn.cards_on_hand))
    ...
    for _ in range(3):
        ...
        if model == 0:
            ...
            person_on_turn.cards_on_hand.remove(card_on_turn)
        ...
        if model == 1:
            ...
            # 如果輪到的是B隊
                ...
                card_on_turn = random_choose(
                person_on_turn, len(person_on_turn.cards_on_hand), suite_for_this_turn)
                opposite_card = card_on_turn.face
...
def bridge_game(model, close_show, num_strategy1, num_strategy2):  # 牌局開始
    p = Poker()  # 建立牌組
    p.shuffle()  # 洗牌
    global count_A_win
...
def control_model():
    ...
    num_strategy1 = 0
    num_strategy2 = 0
    print("\033[7;33m本遊戲共有四位玩家:\033[0m\nA隊伍為：國家機器 及其助手\nB隊伍為：李佳芬   及 韓國瑜\n")
    model = input("請輸入本局型態:\n\t扮演國家機器，和韓總對戰，請輸入 0:\n\
\t電腦自動對戰與策略分析，請輸入 1: \n")
    ...
    if model == "0":
        for person_name in ('國家機器','韓國瑜','國家機器的助手','李佳芬'):
            players.append(Smart(person_name, num_strategy1, num_strategy2))

    if model == "1":
        # print('因為我懶的防錯了，所以這邊不打數字會爆掉，不用試了')
        # number_strategy = int(input('您希望給幾個人添加策略，若不添加預設為隨機出牌。'))
        num = int(input('您希望跑幾次呢？請輸入阿拉伯數字:'))
        for person_name in ('國家機器','韓國瑜','國家機器的助手','李佳芬'):
            print("-"*20)
            print("請為\033[7;33m{}\033[0m賦予策略 ：".format(person_name))
            try:             
                print("\n策略選項\n" + "-"*15)
                print("策略0：隨機出一張")
                ...
                print("策略6：出手中數字最大的牌")
                print("-"*20)
                num_strategy1 = int(input('若\033[7;33m{}\033[0m為首位出牌者，你希望採用第幾號首位出牌策略？請輸入阿拉伯數字:'.format(person_name)))
                print("\n策略選項\n" + "-"*15)
                ...
                num_strategy2 = int(input('若\033[7;33m{}\033[0m並不為首位出牌者，你希望採用第幾號出牌策略？請輸入阿拉伯數字:'.format(person_name)))
                players.append(Smart(person_name, num_strategy1, num_strategy2))
            except:
                print('\n叫你打阿拉伯數字，你打啥小？\n')
                players.clear()
    ...
    return (int(model), num, close_show, num_strategy1, num_strategy2)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print('總共執行了{}次，國家機器隊勝利{}次，勝率為{:.5f}'.format(num_completed, count_A_win, win_ratio))
